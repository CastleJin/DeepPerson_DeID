...
def do_train(
    ...
    for epoch in range(1):
        ...
        for n_iter, (img, vid, target_cam, target_view) in enumerate(train_loader):
            ...
            with torch.no_grad():
                ch_img = modelG(img)
            score_fake, feat_fake = model(
                ch_img, target, cam_label=target_cam, view_label=target_view
            )
            loss_fake = loss_fn(score_fake, feat_fake, target, target_cam)
            loss_D = loss + loss_fake
            ...
            if cfg.MODEL.DIST_TRAIN:
                if dist.get_rank() == 0:
                    torch.save(
                        model.state_dict(),
                        os.path.join(
                            cfg.OUTPUT_DIR, cfg.MODEL.NAME + "my_{}.pth".format(epoch)
                        ),
                    )
            else:
                torch.save(
        ...
        if epoch % 10 == 0:
            torch.save(
                modelG.state_dict(),
                os.path.join(
                    cfg.OUTPUT_DIR, cfg.MODEL.NAME + "G_0.1" + "_{}.pth".format(epoch)
                ),
            )
